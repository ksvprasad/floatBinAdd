...
namespace floatBinAdd
    {
        class Program
            ...
            static int Main(string[] args)
                {   
                    ...
                    if ((num1 > 0) && (num2 > 0))       //A case for two positive numbers.
                        {
                            Sum = p.addition(binaryString1, binaryString2);     //An addition method to actually compute the sum.  
                        }
                    else if ((num1 > 0) && (num2 < 0))  //A case for one positive and one negative number.
                        {
                            if (Math.Abs(num2) > num1)      //To obtain the absolute value of the number (for easy computation).
                                {
                                    Sum = p.subtraction(binaryString2, binaryString1);    //A subtraction method to compute the difference.
                                } 
                            else
                                {
                                    Sum = p.subtraction(binaryString1, binaryString2);    //Binary strings are sent in an order. (bigger number, smaller number).
                                }
                        }
                    else if ((num1 < 0) && (num2 > 0))  //Another case for one positive and one negative number.
                        {
                            if (Math.Abs(num1) > num2)
                                {
                                    Sum = p.subtraction(binaryString1, binaryString2);
                                }
                            else
                                {
                                    Sum = p.subtraction(binaryString2, binaryString1);
                                }
                        }
                    else
                        {
                            Sum = p.addition(binaryString1, binaryString2);
                        }
                    int size = Sum.Length -1;
                    String intFracBin = "";                         //A variable to store both the integral and fractional part of a binary number.
            ...
            string decimalToBinary(double num, int k_precision)
                {
                    ...
                    double fractional = Math.Abs(num) - Integral;   //Subtracting the integral part from the original number gives the fractional part.
                        ...
                        while(k_precision>0)                     //A set precedence is used to denote the number of digits after the decimal point.
                            {
            ...
            int makeEqualLength(ref string binary1, ref string binary2)       //An important method to bring both the strings to equal length
                {                                                          //by appending 0's at the beginning of the string which is smaller in length.
                    int i;                                              
                    int len1 = binary1.Length;                                //Initially the lengths of both the strings are obtained and stored in len1 and len2.
                        ...
                        if(len1 < len2)                         
                            {
                                for(i = 0; i < len2 - len1; i++)                //len2-len1 obtains the difference in lengths if string1's length is less than that of string 2.
                                    binary1 = '0' + binary1;                          //Adds 0 at the beginning if string 1 is smaller in length.   
                        ...
                        else if(len1 > len2)
                            {
                                for(i = 0; i < len1 - len2; i++)                //len1-len2 obtains the difference in lengths if string2's length is less than that of string 1.
                                    binary2 = '0' + binary2;                          //Adds 0 at the beginning if string1 is smaller in length.
            ...
            double binaryToDecimal(BigInteger Binary,int k_precision)        //A method to finally convert back the binary sum to a decimal format.
                { 
                    double Decimal = 0;                                 //A variable to store the result.                        
                    double kprecision = -k_precision;                        //Fractional bits hold powers in negative exponents.
